...
# create the cache and log directories and fix permissions in var folder
RUN mkdir -p var/cache/ && chown -R ${USERNAME}:${USERNAME} var/cache \
    && mkdir -p var/log/ && chown -R ${USERNAME}:${USERNAME} var/log
